...
int wave_radii = 0,wave_width = 150;
int time_to_exit;
...
main(int argc, char **argv)
{
    ...
    printf("popper start = %d popper fade = %d live = %d\n",popper_start,popper_fade,time_to_live);

    compute_radii(rand()%343);
    popper_speed((rand()%5+2)*wave_width);
...
popper_speed(int wave_radii) {
    ...
    for(i=0;i<MAXMOTORS;i++) {
        if(abs(wave_radii-rsquared[i]) < wave_width) {
	    newspeed[map[i]] = 2;
	    speed[map[i]] = 2;
	    motors++;
	}
